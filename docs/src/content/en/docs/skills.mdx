...
For filesystem-based skill discovery across a project, use [workspace skills](/docs/sandbox/skills) instead.
...
Filesystem paths use `LocalSkillSource` under the hood, which reads `SKILL.md` files following the same format as [workspace skills](/docs/sandbox/skills#skillmd-format).
