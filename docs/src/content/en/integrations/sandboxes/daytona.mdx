...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'Record<string, string>',
    description:
      "Daytona Secrets to expose inside the sandbox, mapping environment variable names to Daytona Secret names. The env var holds an opaque placeholder; the real value is substituted into HTTPS request headers at egress toward the Secret's allowed hosts.",
    isOptional: true,
  },
  {
    name: 'computerUse',
