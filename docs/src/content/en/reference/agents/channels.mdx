...
Channels connect agents to messaging platforms. Configure them via the `channels` property on the `Agent` constructor. The object you pass is a `ChannelConfig`. See the [Channels overview](/docs/channels) for concepts and platform setup instructions.
...
de-duplicated so the platform only sees a call when the status changes. When
the run ends, any status it set is cleared automatically so runs that finish
without posting a message don't leave a stale status on the thread.
...
- [Channels overview](/docs/channels): Concepts, quickstart, and platform setup
