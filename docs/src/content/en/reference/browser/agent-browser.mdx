...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Beta option for adding browser recording tools. Provide outputDir to add browser_record and browser_record_caption to the toolset. You can also set maxDurationMs, maxWidth, and maxHeight as defaults for every recording.',
    isOptional: true,
  },
  {
    name: 'excludeTools',
...
When `recording` is configured, `AgentBrowser` also adds the Beta `browser_record` and `browser_record_caption` tools. See [Browser recording (Beta)](/docs/browser#record-browser-sessions).
...
- [Browser overview](/docs/browser): Conceptual guide
- [agent-browser guide](/integrations/browsers/agent-browser): Usage guide
