...
When `recording` is configured, `StagehandBrowser` also adds the Beta `browser_record` and `browser_record_caption` tools. See [Browser recording (Beta)](/docs/browser#record-browser-sessions).
...
- [Stagehand guide](/integrations/browsers/stagehand): Usage guide
