...
description: Learn how to interact with Mastra AI agents, including response generation and streamed interactions, along with agent-tool management using the client-js SDK.
packages:
  - "@mastra/ai-sdk"
  - "@mastra/client-js"
  - "@mastra/core"
...
Stream a response and keep the stream open until every [background task](/docs/harness/background-tasks) dispatched during the run completes. The server re-enters the agentic loop on each task completion so the LLM can react to results in the same call. Requires background tasks to be [enabled on the Mastra instance](/reference/configuration#backgroundtasks) and a memory thread; otherwise the call uses a plain `stream()`.
...
Resume a suspended agent stream with custom data and keep the stream open until every [background task](/docs/harness/background-tasks) dispatched during the run completes. Use this to continue execution after a suspension point, such as a workflow suspend within an agent. Requires background tasks to be [enabled on the Mastra instance](/reference/configuration#backgroundtasks) and a memory thread; otherwise the call uses a plain `resumeStream()`:
...
Use the client SDK schedule methods to manage persisted agent schedules over the `/api/schedules` routes. For concepts and server-side examples, see [Schedules](/docs/harness/schedules) and the [`mastra.schedules` reference](/reference/schedules/overview).
...
Set `durable` to run the agent as a [durable agent](/docs/harness/durable-agents) once the server hydrates it. Pass `true` to accept the defaults, or an object to tune the durable loop:
...
Both `Agent` (code-defined) and `StoredAgent` instances have methods for managing configuration versions. See [Editor versioning](/docs/studio/editor#versioning) for lifecycle and selection behavior.
