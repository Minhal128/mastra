...
<PropertiesTable
  ...
  {
    name: 'loggerOptions',
    type: '{ correlation?: boolean; export?: boolean }',
    description:
      'Controls logger observability behavior. `correlation` injects trace_id/span_id into native log output during traced operations; `export` forwards log records to observability storage.',
    isOptional: true,
    defaultValue: '{ correlation: true, export: true }',
  },
  {
    name: 'idGenerator',
  ...
  },
  {
    ...
    description:
      'Scorers for evaluating agent responses and workflow outputs. Registration also makes a scorer resolvable by ID, which is required to persist its scores. See [Score persistence](/docs/evals/overview#score-persistence)',
    isOptional: true,
    defaultValue: '{}',
  },
  {
  ...
  },
  {
    ...
    description:
      'Configure [background task](/docs/harness/background-tasks) execution for agents. See [background tasks configuration reference](/reference/configuration#backgroundtasks) for all options.',
    isOptional: true,
    properties: [
      {
        type: 'BackgroundTaskManagerConfig',
  ...
  },
  {
    ...
    description:
      'Boot-time recovery behavior for orphaned agent and workflow runs. See [Crash recovery](/docs/harness/durable-agents#crash-recovery).',
    isOptional: true,
        ...
        parameters: [
          {
            ...
            defaultValue: "'off'",
            description:
              "Set to `'auto'` to automatically re-drive orphaned RUNNING durable agent runs on server boot. Recovery re-issues LLM calls and re-executes tool calls, so tools must be idempotent. See [Crash recovery](/docs/harness/durable-agents#crash-recovery).",
          },
        ],
      },
    ],
