...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'boolean',
    description:
      'Whether `subscribeFromOffset()` honors numeric offsets. Defaults to `false`. A non-zero offset on an unsupported implementation falls back to replay subscription and emits a diagnostic warning when a logger is configured.',
  },
]}
/>
...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Starting position used when a backend creates a new consumer group. `"earliest"` includes retained events. `"latest"` receives only events published after group creation. Existing groups keep their checkpoint.',
    isOptional: true,
