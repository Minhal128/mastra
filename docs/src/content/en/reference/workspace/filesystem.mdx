...
Native recursive tree walk executed by the provider in as few calls as possible. When implemented, the workspace `list_files` tool uses it instead of issuing one `readdir` round trip per directory, which matters for remote providers (sandboxes, object stores). If `walk` throws, tools fall back to the `readdir`-based walk automatically.
