...
Pass a reusable template builder to `PlatformSandbox`. A fresh sandbox start derives its deterministic ID and starts or reuses the provider build. Sandbox creation retries while the build is pending:
...
`Template()` supports `runCmd`, `setWorkdir`, `setEnvs`, `aptInstall`, `pipInstall`, and `npmInstall`. Its `id()` method returns the SHA-256 digest of the canonical definition. Platform serializes the builder internally. It replays operations for E2B and converts them to equivalent Railway template operations.
...
The resolver doesn't run when `PlatformSandbox` reattaches to an existing `sandboxId`. It resolves the repository's public default-branch head at fresh-start time, then Platform starts or reuses the corresponding template build. If the repository head can't be resolved or the provider build fails, sandbox creation continues with the provider's default template so runtime setup can perform a cold checkout. The lazy definition never includes credentials. Private-repository template builds require a future secret transport capability.

Platform stores build state under the template's deterministic ID and binds it to the authenticated organization, project, environment, and provider.
