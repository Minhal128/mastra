...
const test13 = agent.generate({
  messages: 'Just a message',
})

const test14 = agent.stream({
  messages: 'Just a message',
})

const test15 = agent.network({
  messages: 'Just a message',
})

// POSITIVE TEST CASE - messages with variable
const myMessages = [{ role: "user", content: "Hello" }];
const test16 = agent.generate({
  messages: myMessages,
  memory: {
    resource: 'resource-id',
    thread: 'thread-id',
