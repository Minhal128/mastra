...
describe('createSubagentTool requestContext forwarding', () => {
  ...
  it('forwards a copy of requestContext with threadId/resourceId stripped', async () => {
    ...
    // Build a requestContext with controller data including threadId/resourceId
    const requestContext = new RequestContext();
    const controllerCtx: Partial<AgentControllerRequestContext> = {
      emitEvent: vi.fn(),
      threadId: 'parent-thread-123',
      resourceId: 'parent-resource-456',
    };
    requestContext.set('controller', controllerCtx);
    ...
    // AgentController context should have threadId/resourceId cleared
    const subagentController = subagentCtx.get('controller') as Partial<AgentControllerRequestContext>;
    expect(subagentController.threadId).toBeNull();
    expect(subagentController.resourceId).toBe('');
    // Other controller fields should be preserved
    expect(subagentController.emitEvent).toBe(controllerCtx.emitEvent);
    expect(result.isError).toBe(false);
  });

  it('forwards requestContext copy when controller context is not set', async () => {
    mockStream.mockResolvedValue(createMockStreamResponse('result text'));
    ...
    // RequestContext without controller data — still should be forwarded
    const requestContext = new RequestContext();
  ...
  it('returns partial subagent output when the parent aborts during streaming', async () => {
    ...
    const requestContext = new RequestContext();
    requestContext.set('controller', { emitEvent, abortSignal: abortController.signal });
  ...
  it('does not default maxSteps when stopWhen is configured', async () => {
    ...
    const requestContext = new RequestContext();
    requestContext.set('controller', { emitEvent: vi.fn() });
...
describe('createSubagentTool forked subagent behavior', () => {
  ...
  it('forks: clones the parent thread and streams on the parent agent when forked=true', async () => {
    ...
    expect(controllerCtx.emitEvent).toHaveBeenCalledWith(
      expect.objectContaining({
        type: 'subagent_start',
    ...
    const subagentController = streamOpts.requestContext.get('controller') as Partial<AgentControllerRequestContext>;
    expect(subagentController.threadId).toBe('forked-thread-1');
    expect(subagentController.resourceId).toBe('parent-resource-1');
  });
  ...
  it('returns an error when forked=true but no parent agent is available', async () => {
    ...
    const tool = createSubagentTool({
      ...
      resolveModel,
      fallbackModelId: 'test-model',
      // getParentAgent omitted — simulates a controller without a current agent.
      cloneThreadForFork,
    });
  ...
  it('returns an error when forked=true but there is no active parent thread', async () => {
    ...
    const requestContext = new RequestContext();
    // controllerCtx without threadId
    requestContext.set('controller', { emitEvent: vi.fn(), resourceId: 'rid' });
  ...
  it('forks: inherits parent toolsets via getParentToolsets and patches `subagent` to a runtime no-op (preserving prompt-cache prefix)', async () => {
    // Forks must inherit controller-injected tools (`ask_user` / `submit_plan` /
    // user-configured controller tools) — the parent Agent's base config doesn't
    // carry them; they're injected per-stream by the controller.
    //
    ...
    expect(forkRequestContext.get('controller')).toMatchObject({ threadId: 'fork-with-toolsets', resourceId: 'rid' });
    ...
    expect(streamOpts.toolsets.controllerBuiltIn.ask_user).toBe(askUser);
    expect(streamOpts.toolsets.controllerBuiltIn.submit_plan).toBe(submitPlan);
    expect(streamOpts.toolsets.controller.view).toBe(userTool);
    ...
    const patchedSubagent = streamOpts.toolsets.controllerBuiltIn.subagent;
    ...
    const patchedTaskWrite = streamOpts.toolsets.controllerBuiltIn.task_write;
    ...
    const patchedTaskCheck = streamOpts.toolsets.controllerBuiltIn.task_check;
  ...
  it('forks: omitting getParentToolsets keeps `toolsets` unset on the stream call (back-compat)', async () => {
    // If a controller doesn't wire getParentToolsets, the fork should still run
    // (just without inherited controller toolsets) — no breaking change.
  ...
  it('non-forked path is unchanged: strips threadId/resourceId and constructs a fresh Agent', async () => {
    ...
    const streamOpts = mockStream.mock.calls[0]![1];
    const controller = streamOpts.requestContext.get('controller') as Partial<AgentControllerRequestContext>;
    expect(controller.threadId).toBeNull();
    expect(controller.resourceId).toBe('');
