...
const setupHandlers = () => {
  server.use(
    http.get('*/api/datasets', () =>
      HttpResponse.json({ datasets: [dataset], pagination: { total: 1, page: 0, perPage: 100, hasMore: false } }),
    ),
...
const renderReview = async () => {
  ...
  const utils = renderWithProviders(
    <PlaygroundModelProvider>
      <ReviewQueueProvider>
        <AgentPlaygroundReview agentId="chef-agent" />
      </ReviewQueueProvider>
    </PlaygroundModelProvider>,
  );
...
describe('AgentPlaygroundReview keyboard navigation', () => {
