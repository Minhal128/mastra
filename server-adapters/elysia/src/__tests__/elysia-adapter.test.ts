...
describe('Elysia Server Adapter', () => {
  ...
  describe('SSE stream handshake', () => {
    ...
    it('writes an initial SSE comment when sseFlushOnConnect is true', async () => {
      ...
      app.derive(adapter.createContextMiddleware());
      await adapter.registerRoute(app, testRoute, { prefix: '' });

      const response = await app.fetch(new Request('http://localhost/test/waiting-stream'));
      ...
      try {
        ...
        if (firstChunk.value instanceof Uint8Array) {
          ...
          expect(String(firstChunk.value)).toBe(': connected\n\n');
        }
      } finally {
        await reader.cancel();
      }
    ...
    it('does not write an initial SSE comment when sseFlushOnConnect is not set', async () => {
      ...
      const response = await app.fetch(new Request('http://localhost/test/waiting-stream-no-flush'));
      const reader = response.body!.getReader();
  ...
  createMultipartTestSuite({
    suiteName: 'Elysia Multipart FormData',
    ...
    startServer: async (app: Elysia) => {
      return startElysiaServer(app);
    },
  ...
  describe('OpenAPI Spec', () => {
    ...
    it('should serve the OpenAPI spec at /api/openapi.json', async () => {
      ...
      const prefixedResponse = await fetch(`${baseUrl}/api/openapi.json`);
  ...
  describe('Custom API Routes (registerApiRoute)', () => {
    ...
    it('should allow custom routes at paths not starting with the server prefix', async () => {
      ...
      const response = await fetch(`${baseUrl}/custom/hello`);
  ...
  describe('Reserved context key injection prevention', () => {
    ...
    it('should not set reserved Studio context when x-mastra-client-type is not studio', async () => {
      ...
      for (const headers of [{ 'x-mastra-client-type': 'playground' }, {}] as Record<string, string>[]) {
        const response = await app.fetch(
          new Request('http://localhost/test/context', {
            method: 'GET',
            headers,
  ...
  createBodyLimitTestSuite({
    ...
    executeRequest: async (app, method, url, options = {}) => {
      // Real HTTP clients send Content-Length; Request() alone does not set it,
      // so mirror that here for the adapter's body-limit gate.
