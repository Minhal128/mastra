...
- Added a `consistency` option to the Turbopuffer vector store. Queries previously always used strong consistency, which adds latency. You can now opt into eventual consistency for lower-latency queries, either for all queries via the constructor or per query. ([#19627](https://github.com/mastra-ai/mastra/pull/19627))
