...
export class ScoresValkey extends ScoresStorage {
  ...
  public async getScoreById({ id }: { id: string }): Promise<ScoreRowData | null> {
    try {
      ...
      throw new MastraError(
        {
          id: createStorageErrorId('VALKEY', 'GET_SCORE_BY_ID', 'FAILED'),
  ...
  public async saveScore(score: SaveScorePayload): Promise<{ score: ScoreRowData }> {
    ...
    try {
      ...
      throw new MastraError(
        {
          id: createStorageErrorId('VALKEY', 'SAVE_SCORE', 'VALIDATION_FAILED'),
          domain: ErrorDomain.STORAGE,
    ...
    try {
      ...
      throw new MastraError(
        {
          id: createStorageErrorId('VALKEY', 'SAVE_SCORE', 'FAILED'),
          domain: ErrorDomain.STORAGE,
          category: ErrorCategory.THIRD_PARTY,
          details: { id },
        },
